...
# File handler
log_file = os.path.join(log_dir, 'stress_analysis.log')
...
# Emotion stress mapping for confidence checks
EMOTION_STRESS_MAP = {
...
def load_custom_model():
    """Load the custom CNN model with proper compatibility handling."""
    ...
    if _custom_model is None:
        try:
            ...
            logger.info("Please ensure the model file is compatible with TensorFlow 2.15.0")
            return False
    return True
...
def analyze_image_array(img_array) -> Dict[str, Any]:
    """
    Analyze a numpy image array for emotion detection and stress analysis
    Args:
        img_array: Numpy array image (BGR format from OpenCV)
    ...
    try:
        ...
        if img_array is None:
            logger.error("Invalid image array: None")
            return {"error": "Invalid image data provided"}
        
        if not isinstance(img_array, np.ndarray):
            logger.error(f"Invalid image type: {type(img_array)}")
        ...
        logger.info(f"Image array validated successfully. Shape: {img.shape}")
        ...
        logger.info(f"Successfully processed image array. Stress level: {stress_level}")
        return result
        
    except Exception as e:
        logger.error(f"Error analyzing image array: {str(e)}", exc_info=True)
...
def analyze_image(image_data: str) -> Dict[str, Any]:
    """
    Analyze a base64 encoded image for emotion detection and stress analysis
    Args:
    ...
    try:
        ...
        # Use the existing analyze_image_array function
        return analyze_image_array(img)
        
    except Exception as e:
        logger.error(f"Error analyzing base64 image: {str(e)}", exc_info=True)
        return {"error": "An unexpected error occurred analyzing the image."}
